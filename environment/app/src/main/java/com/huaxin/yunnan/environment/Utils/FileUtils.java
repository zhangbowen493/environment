...
/**
 * 文件工具类
 * Created by zbw on 2017/11/28 11:50.
 */
...
public class FileUtils {
	...
	//判断文件是否存在,存在就删掉
	public void isFileExist(String fileName, String dir) {
		File file = new File(SDCardRoot + dir + File.separator + fileName);
		LogUtils.e("文件路径1：" + SDCardRoot + dir + File.separator + fileName);
		if (file.exists()) {
			file.delete();
			LogUtils.e("文件存在，执行删除并重新下载");
		} else {
			LogUtils.e("文件不存在，执行下载");
		}
	}
//	//判断文件是否存在
